...
#include "llack/PassManagers.h"
#include "llack/Module.h"
#include "llack/ModuleProvider.h"
#include "llack/Analysis/Dominators.h"
...
// -time-passes is enabled on the command line.
//
bool llack::TimePassesIsEnabled = false;
static cl::opt<bool,true>
...
LlackPassManagerRef LlackCreatePassManager() {
  return wrap(new PassManager());
}

LlackPassManagerRef LlackCreateFunctionPassManager(LlackModuleProviderRef P) {
  return wrap(new FunctionPassManager(unwrap(P)));
}

int LlackRunPassManager(LlackPassManagerRef PM, LlackModuleRef M) {
  return unwrap<PassManager>(PM)->run(*unwrap(M));
}

int LlackInitializeFunctionPassManager(LlackPassManagerRef FPM) {
  return unwrap<FunctionPassManager>(FPM)->doInitialization();
}

int LlackRunFunctionPassManager(LlackPassManagerRef FPM, LlackValueRef F) {
  return unwrap<FunctionPassManager>(FPM)->run(*unwrap<Function>(F));
}

int LlackFinalizeFunctionPassManager(LlackPassManagerRef FPM) {
  return unwrap<FunctionPassManager>(FPM)->doFinalization();
}

void LlackDisposePassManager(LlackPassManagerRef PM) {
  delete unwrap(PM);
}
